...
{
	FingerCollectionAdapter::FingerCollectionAdapter(const NFTemplate& templ) : m_template(templ)
	{
		m_collectionString.Add(wxT("(Collection)"));
	}

	FingerCollectionAdapter::~FingerCollectionAdapter()
	{
	}

	void FingerCollectionAdapter::AddItemCollection(wxListCtrl* ctrl, wxPropertyGridManager* grid, int index)
	{
		NFingerRecordDlg addFRecDlg(NULL, false);

		if (addFRecDlg.ShowModal() == wxID_OK)
		{
			m_listViewString = N_TMPL_NFREC;
			m_template.GetRecords().Add(NFRecord(false, addFRecDlg.GetWidth(), addFRecDlg.GetHeight(), addFRecDlg.GetHorizontalResolution(), addFRecDlg.GetVirticalResolution()));
	...
	void FingerCollectionAdapter::SwapItemCollection(wxListCtrl* ctrl, wxPropertyGridManager* grid, int index1, wxString s1, wxString s2)
	{
	...
	void FingerCollectionAdapter::UpdateItemCollection(wxListCtrl* ctrl, wxPropertyGridManager* grid, int index)
	{
	...
	void FingerCollectionAdapter::ShowItemCollectionInGrid(wxPropertyGridManager* grid, int index)
	{
		...
		{
			if (NBiometricTypes::IsImpressionTypeFinger((NFImpressionType)impressionsValues[i])) impressionTypeChoices.Add(NEnum::ToString(NBiometricTypes::NFImpressionTypeNativeTypeOf(), impressionsValues[i]), impressionsValues[i]);
		}

		for (int i = 0; i < positionValues.GetCount(); i++)
		{
			if (NBiometricTypes::IsPositionFinger((NFPosition)positionValues[i])) positionChoices.Add(NEnum::ToString(NBiometricTypes::NFPositionNativeTypeOf(), positionValues[i]), positionValues[i]);
		}
		...
		NTemplate ntemplate;
		ntemplate.SetFingers(m_template);

		pgrid->Clear();
		pgrid->Append(new wxPropertyCategory(N_TMPL_NFREC, wxPG_LABEL));
		pgrid->Append(new wxIntProperty(N_TMPL_CBEFF_PRODUCT_TYPE, wxPG_LABEL, record.GetCbeffProductType()));
		pgrid->Append(new NCollectionProperty(N_TMPL_CORES, wxPG_LABEL, m_collectionString, ntemplate, FINGER_CORE_COLLECTION, index));
		pgrid->LimitPropertyEditing(N_TMPL_CORES, true);
		pgrid->Append(new NCollectionProperty(N_TMPL_DELTAS, wxPG_LABEL, m_collectionString, ntemplate, FINGER_DELTA_COLLECTION, index));
		pgrid->LimitPropertyEditing(N_TMPL_DELTAS, true);
		pgrid->Append(new NCollectionProperty(N_TMPL_DOUBLE_CORES, wxPG_LABEL, m_collectionString, ntemplate, FINGER_DOUBLE_CORE_COLLECTION, index));
		...
		pgrid->Append(new NCollectionProperty(N_TMPL_MINUTIAE, wxPG_LABEL, m_collectionString, ntemplate, FINGER_MINUTIAE_COLLECTION, index));
		pgrid->LimitPropertyEditing(N_TMPL_MINUTIAE, true);
		pgrid->Append(new NCollectionProperty(N_TMPL_MINUTIAE_NEIGHB, wxPG_LABEL, m_collectionString, ntemplate, FINGER_MINUTIAE_NEIGHBOUR_COLLECTION, index));
		...
		pgrid->Append(new NCollectionProperty(N_TMPL_POSSIBLE_POS, wxPG_LABEL, m_collectionString, ntemplate, FINGER_POSSIBLE_POSITION_COLLECTION, index));
	...
	int FingerCollectionAdapter::GetItemsCount()
	{
		return m_template.GetRecords().GetCount();
	}

	void  FingerCollectionAdapter::DeleteItem(int index)
	{
		m_template.GetRecords().RemoveAt(index);
	}

	wxString FingerCollectionAdapter::PropertyValueChanged(wxString propertyName, int index, wxVariant value)
	{
		NFRecord record = m_template.GetRecords().Get(index);
	...
	void FingerCollectionAdapter::ShowCollectionInList(wxListCtrl* ctrl, int index, wxString listString)
	{
		wxListItem item;
		item.SetId(index);
