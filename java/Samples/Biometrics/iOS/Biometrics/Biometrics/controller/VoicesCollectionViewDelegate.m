...
@implementation VoicesCollectionViewDelegate
...
-(HNSubject)getSubjectAtIndex:(int)index{
    if ((index >= 0) && (index <= voices.count-1)){
        BiometricsResource *resource = [voices objectAtIndex:index];
        return resource.subject;
    }
    return NULL;
}
-(void)reloadData{
    voices = [[BiometricsLayer sharedInstance] getAllSubjectsWithPrefix:PREFIX_VOICE];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    return voices.count>0?voices.count:1;
}
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section{
    if (voices.count == 0) return UIEdgeInsetsZero;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionViewArg cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setupCellWithTitle:voice.name andAudioPath:[voice getAudioFilePath] andType:cellType  canDelete:viewController.navigationController.viewControllers.count>1];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Voice *voice = [voices objectAtIndex:indexPath.row];
    [[BiometricsLayer sharedInstance] fillSubjectWithAttributes:voice.subject];
    if ([viewController.delegate respondsToSelector:@selector(databaseController:didChooseVoice:)]){
        [viewController.delegate databaseController:viewController didChooseVoice:voice];
    }
}
@end
