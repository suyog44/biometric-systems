...
public final class ANType6RecordCreationFrame extends ANImageBinaryRecordCreationFrame {
	...
	protected ANRecord onCreateRecord(ANTemplate template) {
		ANType6Record record = null;
		if (isCreateFromImage()) {
			NImage image = getImage();
			if (image != null) {
				record = new ANType6Record(ANTemplate.VERSION_CURRENT, getIdc(), isIsrFlagUp(), (ANBinaryImageCompressionAlgorithm) getCompressionAlgorithm(), image);
			}
		} else {
