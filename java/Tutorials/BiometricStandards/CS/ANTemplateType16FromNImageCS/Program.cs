...
namespace Neurotec.Tutorials
{
	class Program
	{
		private static int Usage()
		{
			...
			Console.WriteLine("\t{0} [NImage] [ANTemplate] [Tot] [Dai] [Ori] [Tcn] [Src] [Udi]", TutorialUtils.GetAssemblyName());
			Console.WriteLine("");
		...
		static int Main(string[] args)
		{
			...
			try
			{
				...
				using (NImage rgbImage = NImage.FromImage(NPixelFormat.Rgb8U, 0, image))
				{
					...
					var record = new ANType16Record(ANTemplate.VersionCurrent, 0, udi, src, BdifScaleUnits.None, ANImageCompressionAlgorithm.None, rgbImage);
