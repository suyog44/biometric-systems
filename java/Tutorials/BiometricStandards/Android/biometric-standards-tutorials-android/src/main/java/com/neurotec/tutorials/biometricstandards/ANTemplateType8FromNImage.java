...
public final class ANTemplateType8FromNImage extends Activity implements LicensingStateCallback {
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			ANType8Record record = new ANType8Record(ANTemplate.VERSION_CURRENT, 0, ANSignatureType.OFFICIAL, ANSignatureRepresentationType.SCANNED_UNCOMPRESSED, true, image);
			template.getRecords().add(record);

			// Save template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-8.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 8, outputFile.getAbsolutePath()));
		} finally {
