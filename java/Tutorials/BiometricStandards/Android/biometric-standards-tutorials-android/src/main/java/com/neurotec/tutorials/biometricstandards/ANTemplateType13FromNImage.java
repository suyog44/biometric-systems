...
public final class ANTemplateType13FromNImage extends Activity implements LicensingStateCallback {

	private static final String TAG = ANTemplateType13FromNImage.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_antemplate_type_13_from_nimage);
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			ANType13Record record = new ANType13Record(ANTemplate.VERSION_CURRENT, 0, BDIFFPImpressionType.LIVE_SCAN_PLAIN, mSrc, BDIFScaleUnits.PIXELS_PER_INCH, ANImageCompressionAlgorithm.NONE, image);
			template.getRecords().add(record);

			// Save template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-13.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 13, outputFile.getAbsolutePath()));
		} finally {
