...
public final class ANTemplateToNTemplate extends Activity implements LicensingStateCallback {
	...
	private void convert(Uri anTemplateUri) throws IOException {
		...
		try {
			...
			anTemplate = new ANTemplate(IOUtils.toByteBuffer(this, anTemplateUri), ANValidationLevel.STANDARD);

			// Convert ANTemplate object to NTemplate object
			nTemplate = anTemplate.toNTemplate();

			// Save converted template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "ntemplate-from-antemplate.dat");
