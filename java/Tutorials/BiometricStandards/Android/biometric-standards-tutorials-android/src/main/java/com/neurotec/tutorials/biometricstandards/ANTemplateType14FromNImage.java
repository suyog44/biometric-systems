...
public final class ANTemplateType14FromNImage extends Activity implements LicensingStateCallback {

	private static final String TAG = ANTemplateType14FromNImage.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_antemplate_type_14_from_nimage);
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			ANType14Record record = new ANType14Record(ANTemplate.VERSION_CURRENT, 0, mSrc, BDIFScaleUnits.PIXELS_PER_INCH, ANImageCompressionAlgorithm.NONE, grayImage);
			template.getRecords().add(record);

			// Save template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-14.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 14, outputFile.getAbsolutePath()));
		} finally {
