...
public final class ANTemplateType4FromNImage extends Activity implements LicensingStateCallback {
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-4.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 4, outputFile.getAbsolutePath()));
		} finally {
