...
public final class ANTemplateType17FromNImage extends Activity implements LicensingStateCallback {

	private static final String TAG = ANTemplateType17FromNImage.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_antemplate_type_17_from_nimage);
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			ANType17Record record = new ANType17Record(ANTemplate.VERSION_CURRENT, 0, mSrc, BDIFScaleUnits.PIXELS_PER_INCH, ANImageCompressionAlgorithm.NONE, rgbImage);
			template.getRecords().add(record);

			// Save template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-17.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 17, outputFile.getAbsolutePath()));
		} finally {
