...
public final class ANTemplateType16FromNImage extends Activity implements LicensingStateCallback {
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try {
			...
			ANType16Record record = new ANType16Record(ANTemplate.VERSION_CURRENT, 0, mUdi, mSrc, BDIFScaleUnits.NONE, ANImageCompressionAlgorithm.NONE, rgbImage);
			template.getRecords().add(record);

			// Save template to file
			File outputFile = new File(BiometricStandardsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "antemplate-type-16.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), template.save());

			showMessage(getString(R.string.format_antemplate_saved_to, 16, outputFile.getAbsolutePath()));
		} finally {
