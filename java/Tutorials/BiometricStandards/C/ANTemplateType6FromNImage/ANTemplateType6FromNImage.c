	...
	)
{
	...
	result = ANType6RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, isr, ca, hBinImage, 0, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType6RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
