	...
	)
{
	...
	// create grayscale low resolution image from image
	...
	{
		result = NImageSetResolutionIsAspectRatio(hGrayscaleImage, NFalse);
		if(NFailed(result))
		{
	...
	// Create Type 3 record
	result = ANType3RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, isr, ca, hGrayscaleImage, 0, &hRecord);
	if (NFailed(result))
	{
		printf(N_T("error in ANType3RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
	...
	if (NFailed(result))
	{
		printf(N_T("error in ANTemplateAddRecordEx, error code: %d"), result);
		goto FINALLY;
	}
