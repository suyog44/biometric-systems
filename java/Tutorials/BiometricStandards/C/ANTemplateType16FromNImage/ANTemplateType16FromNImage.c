...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [NImage] [ANTemplate] [Tot] [Dai] [Ori] [Tcn] [Src] [Udi]\n"), title);
	printf(N_T("\n"));
	...
	)
{
	...
	HNImage hRGBImage = NULL; // handle to RGB image object
	...
	result = ANType16RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, szUdi, szSrc, slc, cga, hRGBImage, 0, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType16RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType16(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
