...
int usage()
{
	printf(N_T("usage: %s [FIRecord] [NTemplate]\n"), title);
	printf(N_T("\tFIRecord  - input FIRecord\n"));
...
int main(int argc, NChar *argv[])
{
	...
	NInt fv_count;
	NInt i;
	BdifFPPosition bdif_position;
	BdifFPImpressionType bdif_impression_type;
	...
	result = FIRecordCreateFromMemoryN(hBuffer, 0, bsIso, NULL, &hFIRecord);
	if(NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in FIRecordCreateFromMemory, error code: %d\n"), result);
		goto FINALLY;
	}
	...
	result = FIRecordGetFingerViewCount(hFIRecord, &fv_count);
	if(NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in FIRecordGetFingerViewCount, error code: %d\n"), result);
		goto FINALLY;
	}
	...
	{
		result = FIRecordGetFingerView(hFIRecord, i, &hFirFingerView);
		if(NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("error in FIRecordGetFingerView, error code: %d\n"), result);
			goto FINALLY;
		}

		result = FirFingerViewToNImage(hFirFingerView, 0, &hImage);
		if(NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("error in FirFingerViewToNImage, error code: %d\n"), result);
			goto FINALLY;
		}

		result = FirFingerViewGetPosition(hFirFingerView, &bdif_position);
		if(NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("error in FirFingerViewGetPosition, error code: %d\n"), result);
			goto FINALLY;
		}

		result = FirFingerViewGetImpressionType(hFirFingerView, &bdif_impression_type);
		if(NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("error in FirFingerViewGetImpressionType, error code: %d\n"), result);
			goto FINALLY;
		}
		...
		if(NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("error in NFrictionRidgeSetImage, error code: %d\n"), result);
			goto FINALLY;
		}

		result = NFrictionRidgeSetPosition(hFinger, (NFPosition)bdif_position);
		if (NFailed(result))
		{
		...
		result = NFrictionRidgeSetImpressionType(hFinger, (NFImpressionType)bdif_impression_type);
		if (NFailed(result))
		{
