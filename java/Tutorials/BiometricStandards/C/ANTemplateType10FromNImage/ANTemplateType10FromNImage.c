	...
	)
{
	...
	HNImage hRGBImage = NULL; // handle to RGB image
	HANType10Record hRecord = NULL;	// handle to ANRecord object
	NResult result;
	NInt idc = 0; 
	ANImageType imt = anitFace; //image type contained in file, surrently face
	ANImageCompressionAlgorithm cga = anicaNone;	// image compression algortihm currently none
	BdifScaleUnits slc = 0;
	const NChar * szSmt = NULL;
	...
	// Add Type 10 record
	result = ANType10RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, imt, szSrc, slc, cga, szSmt, hRGBImage, 0, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType10RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType10(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
