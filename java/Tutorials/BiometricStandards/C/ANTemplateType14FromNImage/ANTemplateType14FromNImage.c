	...
	)
{
	...
	result = ANType14RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, szSrc, slc, cga, hGrayscaleImage, 0, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType14RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType14(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
