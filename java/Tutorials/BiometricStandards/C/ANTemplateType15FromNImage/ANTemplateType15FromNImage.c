...
const NChar title[] = N_T("ANTemplateType15FromNImage");
const NChar description[] = N_T("Demonstrates creation of ANTemplate with type 15 record in it");
...
NResult ANTemplateAddRecordType15(
	...
	)
{
	...
	HNImage hGrayscaleImage = NULL; // handle to grayscale image
	HANType15Record hRecord = NULL;	// handle to ANRecord object
	NResult result;
	NInt idc = 0; // 
	...
	result = NImageGetHorzResolution(hGrayscaleImage, &horizontalResolution);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("failed to get horizontal resolution for image, error %d\n"), result);
		goto FINALLY;
	...
	result = NImageGetVertResolution(hGrayscaleImage, &verticalResolution);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("failed to get vertical resolution for image, error %d\n"), result);
		goto FINALLY;
	...
	result = ANType15RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, szSrc, slc, cga, hGrayscaleImage, 0, &hRecord);
	if (NFailed(result))
	{
	...
	if(NFailed(result))
	{
		printf(N_T("error in ANTemplateSaveToFile, error code: %d"), result);
		goto FINALLY;
	}
...
{
	NResult result = N_OK;
	const NChar * components = N_T("Biometrics.Standards.Palms");
	...
	result = ANTemplateAddRecordType15(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
