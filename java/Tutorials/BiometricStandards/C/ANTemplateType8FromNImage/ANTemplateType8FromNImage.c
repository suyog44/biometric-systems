...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [Signature] [ANTemplate] [Tot] [Dai] [Ori] [Tcn]\n"), title);
	printf(N_T("\n"));
	...
	)
{
	...
	result = ANType8RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, srt, isr, isrValue, hlrBinImage, N_OBJECT_REF_RET, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType8RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}
...
{
	NResult result = N_OK;
	const NChar * components = N_T("Biometrics.Standards.Other");
	...
	result = ANTemplateAddRecordType8(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
