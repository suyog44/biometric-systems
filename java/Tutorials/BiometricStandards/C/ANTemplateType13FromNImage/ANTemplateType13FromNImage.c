...
const NChar title[] = N_T("ANTemplateType13FromNImage");
const NChar description[] = N_T("Demonstrates creation of ANTemplate with type 13 record in it");
...
NResult ANTemplateAddRecordType13(
	...
	)
{
	...
	HNImage hGrayscaleImage = NULL; // handle to Gray scale image
	HANType13Record hRecord = NULL; // handle to ANRecord object
	NResult result;
	NInt idc = 0;
	ANImageCompressionAlgorithm cga = anicaNone; // image compression algortihm currently none
	BdifScaleUnits slc = bsuPixelsPerInch;
	BdifFPImpressionType imp = bfpitLiveScanPlain; // impression type
	...
	// create grayscale image from an image
	...
	result = NImageSetHorzResolution(hGrayscaleImage, horizontalResolution);
	if (NFailed(result))
	{
	...
	result = NImageSetVertResolution(hGrayscaleImage, verticalResolution);
	if (NFailed(result))
	{
	...
	// Create Type 13 record
	result = ANType13RecordCreateFromNImage(AN_TEMPLATE_VERSION_CURRENT, idc, imp, szSrc, slc, cga, hGrayscaleImage, 0, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANType13RecordCreateFromNImage, error code: %d"), result);
		goto FINALLY;
	}

	// Add Type 13 record to ANTemplate object
	...
	// Store ANTemplate object with type 13 record in file
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType13(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
