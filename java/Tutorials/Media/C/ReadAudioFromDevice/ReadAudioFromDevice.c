...
int usage()
{
	...
	printf(N_T("\n"));
	printf(N_T("\tbufferCount - number of sound buffers to capture from each device to current directory\n"));
	printf(N_T("\n"));
	return 1;
}
...
int main(int argc, NChar **argv)
{
	...
	printf(N_T("quering connected audio devices ...\n"));
	result = NMediaSourceEnumDevices(nmtAudio, 0, &arhDevices, &deviceCount);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("failed to query connected audio devices (result = %d)\n"), result);
		goto FINALLY;
	}
	printf(N_T("done\n"));
	...
	for (i = 0; i < deviceCount; ++i)
	{
		...
		{
			const NChar * szDisplayName = NULL;
		...
		ReadSoundBuffers(hReader, bufferCount);
