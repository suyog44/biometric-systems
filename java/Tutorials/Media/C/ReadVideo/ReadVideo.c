...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [source] [frameCount] <optional: is url>\n"), title);
	printf(N_T("\n"));
	printf(N_T("\tsource - filename or url images should be captured from\n"));
	printf(N_T("\tframeCount - number of images to capture from specified filename or url\n"));
	printf(N_T("\tis url - specifies that passed source parameter is url (value: 1) or filename (value: 0)\n"));
	printf(N_T("\n"));
	return 1;
}

static NResult ReadFrames(HNMediaSource hSource, NInt frameCount);
...
int main(int argc, NChar **argv)
{
	...
	if (frameCount == 0)
	{
		printf(N_T("no frames will be read as frame count is not specified"));
	}
	...
	result = ReadFrames(hReader, frameCount);
...
static NResult ReadFrames(HNMediaReader hReader, NInt frameCount)
{
	NResult result;
	...
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("failed to get clear format (result = %d)!\n"), result);
		goto FINALLY;
	}
	...
	for (j = 0; j < frameCount; j++)
	{
		...
		printf(N_T("[%llu %llu]\n"), timespan, duration);
		sprintf(szFilename, N_T("%d.bmp"), j);
