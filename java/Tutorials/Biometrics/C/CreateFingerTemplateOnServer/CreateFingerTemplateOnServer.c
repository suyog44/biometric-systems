...
const NChar title[] = N_T("CreateFingerTemplateOnServer");
const NChar description[] = N_T("Demonstrates how to create finger template from image on server.");
...
int main(int argc, NChar **argv)
{
	...
	result = NFileReadAllBytesCN(hFingerInput, &hBuffer);
	if (NFailed(result))
	{
	...
	result = NBiometricSetSampleBuffer(hFinger, hBuffer);
	if (NFailed(result))
	{
