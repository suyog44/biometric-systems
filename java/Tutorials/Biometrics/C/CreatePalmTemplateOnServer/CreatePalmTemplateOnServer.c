...
const NChar title[] = N_T("CreatePalmTemplateOnServer");
const NChar description[] = N_T("Demonstrates how to create palm template from image on server.");
...
int main(int argc, NChar **argv)
{
	...
	result = NFileReadAllBytesCN(hPalmInput, &hBuffer);
	if (NFailed(result))
	{
	...
	result = NBiometricSetSampleBuffer(hPalm, hBuffer);
	if (NFailed(result))
	{
