...
const NChar title[] = N_T("IdentifyPalm");
const NChar description[] = N_T("Demonstrates palmprint identification.");
...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [probe image] [one or more gallery images] ... \n"), title);
	printf(N_T("\n"));
...
NResult CreateSubject(HNSubject hSubject, const NChar * fileName, HNString subjectId)
{
	...
	result = NBiometricSetFileName(hPalm, fileName);
	if (NFailed(result))
	{
...
FINALLY:
	{
		NResult result2 = NObjectSet(NULL, &hPalm);
...
int main(int argc, NChar * * argv)
{
...

	{
		...
		// set matching speed
		result = NObjectSetPropertyP(hBiometricClient, N_T("Palms.MatchingSpeed"), N_TYPE_OF(NMatchingSpeed), naNone, &matchingSpeed, sizeof(matchingSpeed), 1, NTrue);
		if (NFailed(result))
		{
			result = PrintErrorMsgWithLastError(N_T("NObjectSetPropertyP() failed (result = %d)!"), result);
			goto FINALLY;
