...
const NChar title[] = N_T("GeneralizeFinger");
const NChar description[] = N_T("Demonstrates template creation and generalization of multiple fingerprints.");
...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [output template] [3 to 10 finger images]\n"), title);
...
int main(int argc, NChar **argv)
{
	...
	for (i = 2; i < argc; i++)
	{
		...
		// set finger image filename
		result = NBiometricSetFileName(hFinger, argv[i]);
		if (NFailed(result))
		{
		...
		result = NBiometricSetSessionId(hFinger, sessionId);
		if (NFailed(result))
		{
