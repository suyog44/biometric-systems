...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [output template] [multiple face images]\n"), title);
...
int main(int argc, NChar **argv)
{
	...
	for (i = 2; i < argc; i++)
	{
		...
		// set face image filename
		result = NBiometricSetFileName(hFace, argv[i]);
		if (NFailed(result))
		{
		...
		result = NBiometricSetSessionId(hFace, sessionId);
		if (NFailed(result))
		{
	...
	{
		printf(N_T("generalazition completed successfully\n"));
