...
const NChar title[] = N_T("IdentifyVoice");
const NChar description[] = N_T("Demonstrates voice identification.");
...
{
	printf(N_T("usage:\n"));
	printf(N_T("\t%s [probe voice] [one or more gallery voices] ... \n"), title);
	printf(N_T("\n"));
...
NResult CreateSubject(HNSubject hSubject, const NChar * fileName, HNString subjectId)
{
	...
	// read and set audio file to voice
...
FINALLY:
	{
		...
		if (NFailed(result2)) PrintErrorMsg(N_T("NLicenseReleaseComponents() failed (result = %d)!"), result2);
	}
