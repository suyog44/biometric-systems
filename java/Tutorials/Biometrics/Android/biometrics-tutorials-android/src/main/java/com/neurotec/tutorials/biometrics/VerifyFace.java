...
public final class VerifyFace extends Activity implements LicensingStateCallback {

	private static final String TAG = VerifyFace.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_verify_face);
	...
	private void init() {
		...
		// Set matching speed
		mBiometricClient.setFacesMatchingSpeed(NMatchingSpeed.LOW);
	}
	...
	private void verify() throws IOException {
		if (!LicensingManager.isFaceMatchingActivated()) {
			showMessage(getString(R.string.format_not_activated, LicensingManager.LICENSE_FACE_MATCHING));
			...
			public void completed(NBiometricStatus result, Void attachment) {
				if (result == NBiometricStatus.OK || result == NBiometricStatus.MATCH_NOT_FOUND) {
					...
					if (result == NBiometricStatus.OK) {
						showMessage(getString(R.string.msg_templates_matched));
					} else {
						showMessage(getString(R.string.msg_templates_not_matched));
					}
				}
			}
