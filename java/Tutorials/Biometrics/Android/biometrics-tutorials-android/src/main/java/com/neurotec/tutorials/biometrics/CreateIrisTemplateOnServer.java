...
public final class CreateIrisTemplateOnServer extends BaseActivity {

	// ==============================================
	// Private static fields
	// ==============================================

	private static final String TAG = CreateIrisTemplateOnServer.class.getSimpleName();
	...
	private void enroll(Uri imageUri) throws IOException {
		...
		try {
			...
			biometricClient.setIrisesTemplateSize(NTemplateSize.SMALL);
			...
			showMessage(String.format("Found %d iriss\n", subject.getIrises().size() - 1));

			status = task.getStatus();

			if (status == NBiometricStatus.OK) {
				// Save template to file.
				File outputFile = new File(BiometricsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "create-iris-template-on-server.dat");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_iris_template_on_server);
