...
public final class GeneralizePalm extends BaseActivity implements LicensingStateCallback {

	private static final String TAG = GeneralizePalm.class.getSimpleName();
	private static final int REQUEST_CODE_GET_IMAGE = 1;
	private static final String[] LICENSES = {LicensingManager.LICENSE_PALM_EXTRACTION};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_generalize_palm);
	...
	private void generalize(List<Uri> images) throws IOException {
		...
		try {
			...
			for (Uri image : images) {
				...
				palm.setImage(NImageUtils.fromUri(this, image));
				palm.setSessionId(1);
			...
			File outputFile = new File(BiometricsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "generalized-palm-template.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), subject.getTemplateBuffer());
			showMessage(getString(R.string.format_generalized_palm_record_saved_to, outputFile.getAbsolutePath()));
		} finally {
