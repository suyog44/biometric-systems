...
public final class GeneralizeFinger extends BaseActivity implements LicensingStateCallback {

	private static final String TAG = GeneralizeFinger.class.getSimpleName();
	private static final int REQUEST_CODE_GET_IMAGE = 1;
	private static final String[] LICENSES = {LicensingManager.LICENSE_FINGER_EXTRACTION};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_generalize_finger);
		...
		mFieldNumber.setHint(R.string.hint_open_images_number_with_range);
	...
	private boolean validateInput() {
		try {
			...
			return mImageCount >= 3 && mImageCount <= 10;
		} catch (NumberFormatException e) {
			showMessage(getString(R.string.format_number_not_valid_with_range, mFieldNumber.getText().toString()));
			return false;
		}
	}
	...
	private void generalize(List<Uri> images) throws IOException {
		...
		try {
			...
			for (Uri image : images) {
				...
				finger.setImage(NImageUtils.fromUri(this, image));
			...
			File outputFile = new File(BiometricsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "generalized-finger-template.dat");
			NFile.writeAllBytes(outputFile.getAbsolutePath(), subject.getTemplateBuffer());
			showMessage(getString(R.string.format_generalized_finger_record_saved_to, outputFile.getAbsolutePath()));
		} finally {
