...
public final class Identify extends BaseActivity implements LicensingManager.LicensingStateCallback {
	private static final String TAG = Identify.class.getSimpleName();
	private static final int REQUEST_CODE_GET_CANDIDATE_RECORD = 1;
	private static final int REQUEST_CODE_GET_PROBE_RECORD = 2;
				...
				if (result.getStatus() == NBiometricStatus.OK) {
					for (NMatchingResult matchingResult : result.getSubjects().get(0).getMatchingResults()) {
						...
						if (matchingResult.getMatchingDetails() != null) {
							showMessage(matchingDetailsToString(matchingResult.getMatchingDetails()));
						}
					}
				} else {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_identify);
		...
		mButtonCandidate.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				getTemplate(REQUEST_CODE_GET_CANDIDATE_RECORD);
			}
		});
		mButtonProbe = (Button) findViewById(R.id.tutorials_button_2);
		mButtonProbe.setText(R.string.msg_select_probe_template);
		mButtonProbe.setEnabled(false);
		mButtonProbe.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				getTemplate(REQUEST_CODE_GET_PROBE_RECORD);
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == REQUEST_CODE_GET_CANDIDATE_RECORD) {
			if (resultCode == RESULT_OK) {
				enroll(data.getData());
			}
		} else if (requestCode == REQUEST_CODE_GET_PROBE_RECORD) {
			if (resultCode == RESULT_OK) {
	...
	private void getTemplate(int requestCode) {
