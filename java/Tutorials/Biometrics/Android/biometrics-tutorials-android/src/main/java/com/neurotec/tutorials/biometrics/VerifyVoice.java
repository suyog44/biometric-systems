...
public class VerifyVoice extends Activity implements LicensingManager.LicensingStateCallback {

	private static final String TAG = VerifyVoice.class.getSimpleName();
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				case OBTAINING:
					...
					mProgressDialog = ProgressDialog.show(VerifyVoice.this, "", getString(R.string.msg_obtaining_licenses));
					break;
				case OBTAINED:
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_verify_voice);
		mButtonReference = (Button) findViewById(R.id.tutorials_button_1);
		mButtonReference.setText(R.string.msg_select_reference_voice);
		...
		mButtonCandidate.setText(R.string.msg_select_candidate_voice);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			try {
				...
				if (mReference != null && mCandidate != null) {
					verify();
				}
			} catch (Exception e) {
	...
	private void verify() throws IOException {
		if (!LicensingManager.isVoiceMatchingActivated()) {
			showMessage(getString(R.string.format_not_activated, LicensingManager.LICENSE_VOICE_MATCHING));
