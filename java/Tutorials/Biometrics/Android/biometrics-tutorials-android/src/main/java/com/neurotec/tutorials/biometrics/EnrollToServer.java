...
public final class EnrollToServer extends BaseActivity {

	private static final String TAG = EnrollToServer.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_enroll_to_server);
	...
	private NSubject createSubject(Uri uri) throws IOException {
		...
		subject.setTemplateBuffer(NBuffer.fromByteBuffer(IOUtils.toByteBuffer(EnrollToServer.this, uri)));
	...
	private void enroll(Uri templateUri) throws IOException {
		...
		try {
			...
			if (task.getStatus() != NBiometricStatus.OK) {
				showMessage(getString(R.string.format_enrollment_unsuccessful, task.getStatus().toString()));
			} else {
				showMessage(getString(R.string.format_enrollment_successful, task.getStatus().toString()));
			}
		} finally {
