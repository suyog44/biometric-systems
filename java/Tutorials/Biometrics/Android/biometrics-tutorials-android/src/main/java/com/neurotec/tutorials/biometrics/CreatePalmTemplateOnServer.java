...
public final class CreatePalmTemplateOnServer extends BaseActivity {
	...
	private static final String TAG = CreatePalmTemplateOnServer.class.getSimpleName();
	...
	private void enroll(Uri imageUri) throws IOException {
		...
		try {
			...
			biometricClient.setPalmsTemplateSize(NTemplateSize.SMALL);
			...
			if (subject.getPalms().size() > 1)
			showMessage(String.format("Found %d palms\n", subject.getPalms().size() - 1));

			status = task.getStatus();

			if (status == NBiometricStatus.OK) {
				// Save template to file.
				File outputFile = new File(BiometricsTutorialsApp.TUTORIALS_OUTPUT_DATA_DIR, "create-palm-template-on-server.dat");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_create_palm_template_on_server);
