...
public final class EnrollFingerFromImage extends BaseActivity implements LicensingStateCallback {

	private static final String TAG = EnrollFingerFromImage.class.getSimpleName();
	private static final int REQUEST_CODE_GET_IMAGE = 1;

	private static final String LICENSE = LicensingManager.LICENSE_FINGER_EXTRACTION;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tutorial_enroll_finger_from_image);
		mFormat = (EditText) findViewById(R.id.tutorials_field_1);
		mFormat.setHint("ISO or ANSI");
	...
	private void enroll(Uri imageUri) throws IOException {
		...
		try {
			...
			if (status == NBiometricStatus.OK) {
				...
				if (mFormat.getText().toString().equals("ISO"))
				{
				...
				else if (mFormat.getText().toString().equals("ANSI"))
				{
					NFile.writeAllBytes(outputFile.getAbsolutePath(), subject.getTemplateBuffer(CBEFFBiometricOrganizations.INCITS_TC_M1_BIOMETRICS,
