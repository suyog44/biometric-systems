...
static const NChar gs_title[] = N_T("Neurotechnology IIR to BIR conversion tutorial 5.1\n");

int print_usage(NChar * argv[])
{
	printf(N_T("Usage: %s <IIR file> <BIR template file> <PurposeID>\n"), argv[0]);
	printf(N_T("\t<IIR file> - serialized FIR file name\n"));
	...
	printf(N_T("\nExample:\n"));
	printf(N_T("%s finger.iir intermediate.bir 1\n"), argv[0]);
...
int ReadIIRFromFile(const NChar * fileName, NByte * * buf, NSizeType * size)
{
	HNBuffer hBuffer = NULL;
	NResult result = NFileReadAllBytesCN(fileName, &hBuffer);
	if (NFailed(result))
	{
		printf(N_T("Could not read IIR file\n"));
		return -1;
	}
...
int main(int argc, NChar * argv[])
{
	...
	if (ReadIIRFromFile(argv[1], &buf, &size) != 0)
	{
	...
	bir.Header.Format.FormatType = CBEFF_BDBFI_ISO_IEC_JTC_1_SC_37_BIOMETRICS_IRIS_IMAGE_RECTILINEAR;
	bir.Header.Format.FormatOwner = CBEFF_BO_ISO_IEC_JTC_1_SC_37_BIOMETRICS;
	bir.Header.FactorsMask = BioAPI_TYPE_IRIS;
