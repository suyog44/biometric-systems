...
static const NChar gs_title[] = N_T("Neurotechnology FCR to BIR conversion tutorial 5.1\n");

int print_usage(NChar * argv[])
{
	printf(N_T("Usage: %s <FCR file> <BIR template file> <PurposeID>\n"), argv[0]);
	printf(N_T("\t<FCR file> - serialized FCR file name\n"));
	...
	printf(N_T("This tutorial converts FCR serialized file to BioAPI BIR serialized file.\n"));
	...
	printf(N_T("\nExample:\n"));
	printf(N_T("%s finger.fcr intermediate.bir 1\n"), argv[0]);
...
int ReadFCRFromFile(const NChar * fileName, NByte * * buf, NSizeType * size)
{
	HNBuffer hBuffer = NULL;
	NResult result = NFileReadAllBytesCN(fileName, &hBuffer);
	if (NFailed(result))
	{
		printf(N_T("Could not read FCR file\n"));
		return -1;
	}
...
int main(int argc, NChar * argv[])
{
	...
	if (ReadFCRFromFile(argv[1], &buf, &size) != 0)
	{
	...
	bir.Header.Format.FormatType = CBEFF_BDBFI_ISO_IEC_JTC_1_SC_37_BIOMETRICS_FACE_IMAGE;
	bir.Header.Format.FormatOwner = CBEFF_BO_ISO_IEC_JTC_1_SC_37_BIOMETRICS;
	bir.Header.FactorsMask = BioAPI_TYPE_FACIAL_FEATURES;
	...
	{
		printf(N_T("Error writing output file\n"));
		NFree(buf);
		return -1;
	}
